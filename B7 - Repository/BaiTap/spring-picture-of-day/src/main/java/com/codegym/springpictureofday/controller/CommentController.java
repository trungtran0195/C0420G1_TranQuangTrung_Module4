...
public class CommentController {
    ...
    public ModelAndView getListComment(@PageableDefault(value = 2) Pageable pageable) {
        return new ModelAndView("comment/list-comment","comments",commentService.getAll(pageable));
    }

    @GetMapping("/update-like/{id}")
    public ModelAndView updateLike(@PathVariable("id") Long id, @PageableDefault(value = 2)Pageable pageable) {
