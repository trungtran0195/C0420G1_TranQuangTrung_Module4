...
public class CinemaDiscountController {
    ...
    @GetMapping("/discount")
    public ModelAndView listDiscounts(@PageableDefault(2) Pageable pageable, @RequestParam(value = "search",defaultValue = "")String search, @RequestParam(value = "by",defaultValue = "all")String by) {
        ...
        if (cinemaDiscounts.isEmpty()) {
            return new ModelAndView("list", "message", "Không tìm thấy dữ liệu trong DB");
        } else {
